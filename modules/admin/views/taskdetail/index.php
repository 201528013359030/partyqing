...
function getCheckedzp()   //指派
{
    ...
    for (var i = 0; i < notes.length; i++)
    {
	 ...
	 $.get('index.php?r=admin/taskdetail/savezp',{uid:id,id:<?=$taskId?>},function(data){
	    	//alert(JSON.stringify(data));
	        if(data){
	        	//alert(JSON.stringify(data.tree));
	           ...
	           $("#announceform-receiverzp").css("display","none");
	           $("#cancel").css("display","block"); 
	           $("#name").css("display","block"); 
	           document.getElementById('name0').innerHTML=notes[0].data.text;
	        }else{
	            alert('error');
	        }
	    },'json');
...
<div id="wrap" class="wrap">
	...
	<form name="send_form" id="send_form" method="post" enctype="multipart/form-data" action="index.php?r=admin/taskdetail/save">   
	<div class="moGrid">
		<div class="formBox">			
   <div class="moGrid" style="padding-bottom:0px;width:100%">		
		<div class="header">
			<h4><?=$info['content']?>
			<?php if($instead==1):?>
			<span style="font-size: 11pt;color:#727272">(<?=$name?>)</span>
			<?endif?>
			</h4>
			<span class="fl" style="margin-top:10px;color:#727272">考核时间：<?=$info['starttime']?>至<?=$info['endtime']?></span> 			
		    <span class="fr" id="name" style="margin-top:10px;color:#727272">已指派给：<span id="name0"><?//=$name?></span></span>
		    <?php if($zp==0):?>
	...
	<hr>	
	<div class="moGrid" id="cancel" onclick="cancel()">
		<div class="btnBox">                    
			<a href="javascript:void(0);" class="btnIteam btnSubmit" style="text-decoration:none;background-color: #ff7777;width:90%;-webkit-border-radius:13px;border-radius: 13px;">							
            <span class="ic_text" style="display:inline-block;line-height: 2.9;height: 2.9em;font-size:12pt;">取消指派</span>
			</a>						
		</div>
</div>
<div id="detail1" style="display:">				
			<div class="inpBox">				
			<span style="display:inline-block;top:10px">汇报内容：</span><textarea id="announceform-content" class="txtare" name="AnnounceForm[content]" style="-webkit-tap-highlight-color:rgba(0,0,0,0);height: 150px;width:75%" placeholder="请输入汇报的具体内容（必填）" onpropertychange="this.style.height=this.scrollHeight + 'px'" oninput="this.style.height=this.scrollHeight + 'px'"></textarea>		            		
			</div>			
			<!--	描述：上传附件的按钮            -->
			...
			<!--end 结束附件框-->
            <hr>
            <h4 style="margin-bottom: 15px;">汇报对象<em style="font-size:11pt">(可不选)</em></h4>
			<div  onclick="receiverSelects();" id="announceform-receiver" class="inpBox nameList" style="cursor:pointer;" title="点击选择接收者">
		...
		<div class="modal-content">
			...
			<div class="modal-body" style="height:350px;">	
				<div style="width:100%; height:320px; margin:auto; #float:left; clear:both; border:1px solid #ccc; overflow:auto;  ">
					...
					<ul id="treezp" style="min-width:300px;width:300px;line-height:2.5em"></ul>
				</div> 
			</div>
			<div class="modal-footer">
				<button id="closeContactszp" type="button" class="btn btn-default"  data-dismiss="modal" onclick="closeContactsModal()" >关闭
				</button>
...
function cancel(){
		//window.location.replace("index.php?r=admin/taskdetail/cancel&id="+"<?=$taskId?>");
	 $.get('index.php?r=admin/taskdetail/cancel',{id:<?=$taskId?>},function(data){
	    	//alert(JSON.stringify(data));
	        if(data){
	        	//alert(JSON.stringify(data.tree));
