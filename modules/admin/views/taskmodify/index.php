   ...
   <div class="moGrid" style="width:100%">		
	...
	<?php if($attachList!=""):?> 		
		  <?php foreach ($attachList as $key=>$value): ?>
			<div id="attach1" href="#" class="attaIteam" onclick='download_file("<?=$value['path']?>","<?=$value['name']?>","<?=$value['size']?>","<?=$value['url']?>");'>
					<div class="ficon ic_x fr" style='cursor:pointer;' onclick='file_del(1)'></div>
					<i id="attachName1" class="ficon ic_file"></i><?=$value['name']?>
				</div>
			<? endforeach?>			
	<?endif?>	
...
function download_file(file,name,size,url){
   ...
   // return false;
    if(isAndroid || isiOS){
        ...
        download_start(name,url,size);
        window.location.href='index.php?r=admin/taskmodify/download&file='+file+'&name='+name;
    }
    return false;
}
var fileID = 1;
