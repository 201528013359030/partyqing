...
class AnnounceForm extends Model
{
	...
	public function save($data){
        ...
        if($receiverId0[0]==""){
        $sq0="UPDATE task_submit SET reporttime='".$now."',taskcontent='".($data['content'])."',taskfiles='".(($data['attach']))."',taskstate=2 WHERE taskId='".$data['taskId']."'";       	 
        }else{
        $sq0="UPDATE task_submit SET approverId='".$receiverId0[0]."',reporttime='".$now."',taskcontent='".($data['content'])."',taskfiles='".(($data['attach']))."',taskstate=1 WHERE taskId='".$data['taskId']."'";
        }
        //file_put_contents("/usr/local/www/nginx/html/partyqing/wt.txt", date("D M d H:i:s Y") . " =======676767=========" .$sq0."\n", FILE_APPEND);
        
        $command = $connection->createCommand($sq0);
        $command->execute();   
        $sq2="INSERT INTO  task_procedure(taskId,uid,comment,approvertime) VALUES('$data[taskId]','$receiverId0[0]','1','$now')";      
        ...
        $data9["eid"]="$eid[1]";
        $data9["title"]="审批通知";
        $data9["url"]="/partyqing/web/index.php?r=admin/taskdetailed/index&check=1&id=".$data['taskId'];
	...
	public function savealter($data){
		$ws = new IctWebService();
		$receiverType = Yii::$app->request->post('receiverType');
		if(YII_ICT){
			$receiverType = 4;
		...
		//$content = iconv("GBK", "UTF-8", $content);
		$data['attach'] = json_encode($attach);
		//file_put_contents("/usr/local/www/nginx/html/partyqing/wt.txt", date("D M d H:i:s Y") . " =======676767qqqqqqqqq=========" . json_encode($attach)."\n", FILE_APPEND);
		$qq=$data['attach'] ;
		//$data['attach']  = str_replace("\u","\\\u", $qq);
		$data['attach']  = str_replace("\\","\\\\", $qq);
		$data['content'] = str_replace(PHP_EOL, "</br>", $data['content']);
		...
		$connection = Yii::$app->db;
		$connection->open();  //初始化数据库
		$sq0="SELECT taskfiles,editflag,approverId FROM task_submit WHERE taskId='".$data['taskId']."'";
		$command = $connection->createCommand($sq0);
		$con = $command->queryOne();
		//file_put_contents("/usr/local/www/nginx/html/partyqing/wt.txt", date("D M d H:i:s Y") . " =======676767qqqqqqqqqqq=========" . json_encode($con)."\n", FILE_APPEND);
		
		if(is_array(json_decode($data['attach']))){
		if(is_array(json_decode($con['taskfiles']))){
		$data['attach'] = json_encode(
				array_merge(json_decode($data['attach'], true),json_decode($con['taskfiles'], true) )
		);}
		}else{
			$data['attach']=$con['taskfiles'];
		}
		//$data['content'] = '<p>'.$data['content'].'</p>';
		//file_put_contents("/usr/local/www/nginx/html/partyqing/wt.txt", date("D M d H:i:s Y") . " =======676767qqqqqqqqqqq=========" . json_encode($data['attach'])."\n", FILE_APPEND);
		
		header("Content-type:text/html;charset=gbk");
		$now=date('Y-m-d H:i:s',time());
		$receiverId0=explode(',',$data['receiverId']);
		if($data['attach']=="\"\""){$data['attach']="";}
		if($con['editflag']==1){$modify=2;}else{$modify=0;}
		if($receiverId0[0]==""){$receiverId0[0]=$con['approverId'];}
		if($receiverId0[0]==""){
			$sq0="UPDATE task_submit SET reporttime='".$now."',taskcontent='".($data['content'])."',taskfiles='".(($data['attach']))."',taskstate=2,editflag=".$modify." WHERE taskId='".$data['taskId']."'";
		}else{
			$sq0="UPDATE task_submit SET approverId='".$receiverId0[0]."',reporttime='".$now."',taskcontent='".($data['content'])."',taskfiles='".(($data['attach']))."',taskstate=1,editflag=".$modify." WHERE taskId='".$data['taskId']."'";
		}
		//file_put_contents("/usr/local/www/nginx/html/partyqing/wt.txt", date("D M d H:i:s Y") . " =======676767=========" .$sq0."\n", FILE_APPEND);	
		$command = $connection->createCommand($sq0);
		$command->execute();
		if($con['editflag']==0&&$receiverId0[0]){
		$sq2="INSERT INTO  task_procedure(taskId,uid,comment,approvertime) VALUES('$data[taskId]','$receiverId0[0]','1','$now')";
		$command = $connection->createCommand($sq2);
		$command->execute();}
		/************发通知begin**************/
