...
use Rest;
use Client;
class LeamateriallistController extends Controller
{
	public $layout  = false;
...
//         $auth->authTokenSession();  //fyq
        ...
        $connection->open();
        $eid=explode('@',$uid);
        $condition="select * from djleamaterial where top=1 and eid='".$eid[1]."'order by time desc limit 3";
        ...
        //$a=$arr[0]/$arr[1];
        //print_r($a);exit;
      	$condition="select * from djleamaterial where top=0 and eid='".$eid[1]."'order by time desc limit 15";
    	$result = $connection->createCommand($condition);
    ...
    public function actionSearch(){
    	...
    	$uid =\Yii::$app->session['user.uid'];
    	$eid=explode('@',$uid);
    	$sq0="select * from djleamaterial where top=0 and eid='".$eid[1]."'and (title like '%".$searchtitle."%' or keywords like '%".$searchtitle."%') ORDER BY time desc LIMIT 15";
    ...
    public function actionGetdata(){
    	...
    	if(strlen($searchcontent)>0){
    		...
    		$sq0="select * from djleamaterial where top=0 and eid='".$eid[1]."'and title like ('%".$searchcontent."%' or keywords like '%".$searchcontent."%') ORDER BY time desc LIMIT ".$search_count.",15";
