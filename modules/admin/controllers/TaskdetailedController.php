...
$uid =$integralArr['reporterId'];//汇报人
//echo $uid;
if($uid){
$eid=explode('@',$uid);
$uid1=explode(',',$integralArr['approverId']);//审批人
...
//print_r($pc);exit;
$name=$pc['result'][0]['data']['membername'][0];}else{$name="";}
...
if($imgurl==""){$imgurl="../web/img/temp/avatar.png";}
      ...
      // print_r($list);
      if($integralArr['assign']==1){
       $suid=$integralArr['assignuid'];
       ...
       $sname=$con['name'];
    	...
    	return $this->render('index',array('list'=>$list,'content'=>$integralArr,'attachList'=>$attachList,
    	   'a'=>$a,'sname'=>$sname,'attach_count'=>$attachCount,'check'=>$check,'id'=>$id,'suid'=>$suid,'uid'=>$uid,'offline_ip'=>$offline_ip['result']['offline_ip']));
    }
 public function actionCheck(){
    ...
    //	$uid ="7@3";
    	...
    	require_once(dirname(__FILE__)."/config.php");
    	$data9=array();
    		$sq0="UPDATE task_submit SET taskstate=2,approvertime='".$now."' WHERE taskId='".$id."'";
            $command = $connection->createCommand($sq0);
            $command->execute(); 
            $sq2="INSERT INTO  task_procedure(taskId,uid,comment,approvertime,commentcontent) VALUES('$id','$uid','2','$now','$comment')";
            $command = $connection->createCommand($sq2);
            $command->execute();
            
            $sq0="SELECT reporterId FROM task_submit WHERE taskId='".$id."'";
            $command = $connection->createCommand($sq0);
    ...
    public function actionCheckno(){   	 
    	...
    	require_once(dirname(__FILE__)."/config.php");
    	$data9=array();
    		$sq0="UPDATE task_submit SET taskstate=3,approvertime='".$now."' WHERE taskId='".$id."'";
            $command = $connection->createCommand($sq0);
            $command->execute(); 
            $sq2="INSERT INTO  task_procedure(taskId,uid,comment,approvertime,commentcontent) VALUES('$id','$uid','3','$now','$comment')";
            $command = $connection->createCommand($sq2);
            $command->execute();
            $sq0="SELECT reporterId FROM task_submit WHERE taskId='".$id."'";
            $command = $connection->createCommand($sq0);
