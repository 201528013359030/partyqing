...
class FaguilistController extends Controller
{
    ...
    public function actionIndex()
    {
       ...
       // $auth->authTokenSession();
        ...
        $connection->open();
        $condition="select * from djfagui where top=1 order by time desc limit 3";
        ...
        $listtop[0]['id']="";
      	...
      	$condition="select * from djfagui where top=0 order by time desc limit 5";
    	$result = $connection->createCommand($condition);
    ...
    public function actionSearch(){
    	...
    	$sq0="select * from djfagui where top=0 and (title like '%".$searchtitle."%' or keywords like '%".$searchtitle."%') ORDER BY time desc LIMIT 15";    	
    ...
    public function actionGetdata(){
    	...
    	if(strlen($searchcontent)>0){
    		...
    		$sq0="select * from djfagui where top=0 and title like ('%".$searchcontent."%' or keywords like '%".$searchcontent."%') ORDER BY time desc LIMIT ".$search_count.",15";   
    		$search_count=$search_count+15;
    		\Yii::$app->session['search_count']=$search_count;
    		 
    	}else{
    		$public_count=\Yii::$app->session['public_count'];
    		$sq0="select * from djfagui where top=0 ORDER BY time desc LIMIT ".$public_count.",15";   
