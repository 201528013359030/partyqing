...
//    public $layout  = 'layout';    
    ...
    public function actionCheck(){
    	...
    	for($index=0;$index<count($checkall);$index++){
    		$sq0="UPDATE djtask_list SET taskstate=2,approvertime='".$now."' WHERE taskId='".$checkall[$index]."'";
            $command = $connection->createCommand($sq0);
            $command->execute();
            $sq2="INSERT INTO  djtask_procedure(taskId,uid,comment,approvertime) VALUES('$checkall[$index]','$uid','2','$now')";
            $command = $connection->createCommand($sq2);
            $command->execute();
            $sq0="SELECT reporterId FROM djtask_list WHERE taskId='".$checkall[$index]."'";
            $command = $connection->createCommand($sq0);
    ...
    public function actionCheckno(){
    	...
    	$data9=array();
    	for($index=0;$index<count($checkall);$index++){
    		$sq0="UPDATE djtask_list SET taskstate=3,approvertime='".$now."' WHERE taskId='".$checkall[$index]."'";
            $command = $connection->createCommand($sq0);
            $command->execute(); 
            $sq2="INSERT INTO  djtask_procedure(taskId,uid,comment,approvertime) VALUES('$checkall[$index]','$uid','3','$now')";
            $command = $connection->createCommand($sq2);
            $command->execute();
            $sq0="SELECT reporterId FROM djtask_list WHERE taskId='".$checkall[$index]."'";
            $command = $connection->createCommand($sq0);
